...
func TestGetNextElement(t *testing.T) {
	cases := []struct {
		...
		{*strings.NewReader(ACMText), bibElement{"author", "Ahmad, Waqar and Hasan, Osman and Tahar, Sofiene"}},
		{*strings.NewReader(IEEEText), bibElement{"author", "Wang, Wenlong and Liu, Minghui and Zhao, Xicai and Yang, Gui"}},
		{*strings.NewReader(SciDirText), bibElement{"title", "Research and implementation of virtual circuit test tool for smart substations"}},
		{*strings.NewReader(ScopusText), bibElement{"author", "Chamana, Manohar and Bhatta, Rabindra and Schmitt, Konrad and Shrestha, Rajendra and Bayne, Stephen"}},
	}
...
func TestParseEntry(t *testing.T) {
	cases := []struct {
		entry       strings.Reader
		parsedEntry bibEntry
	}{
		{*strings.NewReader(ACMText), bibEntry{
			category:  "article",
			key:       "1",
		...
		{*strings.NewReader(IEEEText), bibEntry{
			category: "article",
			key:      "1",
		...
		{*strings.NewReader(SciDirText), bibEntry{
			category: "article",
			key:      "1",
			title:    "Research and implementation of virtual circuit test tool for smart substations",
			journal:  "Procedia Computer Science",
		...
		{*strings.NewReader(ScopusText), bibEntry{
			category:              "article",
			key:                   "1",
			...
			authorKeywords:        []string{"cyberattacks", "cyber–physical systems", "education", "power systems", "real-time testbed", "smart grids"},
			correspondenceAddress: []string{"R. Bhatta", "National Wind Institute, Texas Tech University, Lubbock, 79401, United States", "email: rabindra.bhatta(at)ttu.edu"},
			publisher:             "Multidisciplinary Digital Publishing Institute (MDPI)",
			issn:                  "20763417",
			language:              "English",
			abbrevSourceTitle:     "Appl. Sci.",
			publicationStage:      "Final",
			source:                "Scopus",
			note:                  "Cited by: 3; All Open Access, Gold Open Access",
		}},
		{*strings.NewReader(MissingTests), bibEntry{
			category:  "article",
			key:       "1",
			abstract:  "This is an abstract",
			coden:     "IEIMA",
